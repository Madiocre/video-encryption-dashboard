      ...
      <TableBody>
        {apiKeys.map((key) => (
          <TableRow key={key.id}>
            ...
            <TableCell><Button variant="destructive" size="icon"><Trash /> </Button></TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
<Button className="ml-auto" variant="outline" onClick={() => setIsApiKeyCreated(true)}>
      ...
      <ApiKeysTable />
      <ApiKeyDialog open={isApiKeyCreated} apiKey="12345" apiSecret="abcde12345" onClose={() => setIsApiKeyCreated(false)} />
    </div>
  );
};
